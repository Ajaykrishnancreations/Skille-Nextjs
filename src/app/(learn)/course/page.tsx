...
import { addDataToFirestore, getUserDatafromFirestore, updateCourseData, addCourseChapterData } from "@/api/Api";
...
export default function LearnPage() {
    ...
    const addData = (e: { preventDefault: () => void; }) => {
        e.preventDefault();
        addDataToFirestore(title, imgUrl, summary, course_id, level, skills, newprice, oldprice).then((added) => {
            if (added) {
                alert("Data added");
                setaddCourse(false);
                const data = {
    ...
    const addCourseChapters = (e: { preventDefault: () => void; }) => {
        e.preventDefault();
        const storednewCourseData = localStorage.getItem("newCourseData");
        const newCourseData = storednewCourseData ? JSON.parse(storednewCourseData) : null;
        addCourseChapterData(chapterTitle, chapterSummary, newCourseData?.course_id, newCourseData?.course_title).then((added) => {
            if (added) {
                alert("Data added");
                setaddCourse(false);
                setValue(true)
    ...
    return (
        <div>
            ...
            {UserData?.role === "creator" && (
                <div className="p-10">
                    <h5 onClick={openModal1}>Add New Course</h5>
                    {
                        isModalOpen1 && (
